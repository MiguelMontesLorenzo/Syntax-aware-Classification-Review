...
class SerialNaiveBayes:
    ...
    def fit(
        ...
        sigma: int = 1
        numerator: torch.Tensor = words_tensor + sigma
        denominator: torch.Tensor = (
            torch.sum(words_tensor, dim=0) + sigma * self.vocab_size
        )
        words_prob: torch.Tensor = numerator / denominator
        class_prob: torch.Tensor = class_tensor / torch.sum(class_tensor, dim=0)
    ...
    def predict_probabilities(self, sentence: torch.Tensor) -> torch.Tensor:
        ...
        conditional_log_probs: torch.Tensor = torch.einsum(
            "ij,i->j", self.log_words_tensor, sentence
        )
        log_probs: torch.Tensor = conditional_log_probs + self.log_class_tensor
        return log_probs
    ...
    def predict(self, sentence: torch.Tensor) -> int:
        ...
        log_probs: torch.Tensor = self.predict_probabilities(sentence)
