...
from src.Embeddings.data_processing import (
...
def main():
    ...
    if train_model:
        ...
        train_words, freqs, sampled_correspondences = subsample_words(
            tokens, vocab_to_int, correspondences
        )
        print(f"Subsampled words to {len(train_words)} training examples.")

        print("Step 4: Creating DataLoader...")
        dataloader: DataLoader = generate_data_loader(
            train_words,
            sentences,
        ...
        train_skipgram(
            model, dataloader, int_to_vocab, epochs, learning_rate, print_every, device
        )
